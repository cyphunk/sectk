...
27/12/2006 Thomas F. Divine <tdivine@pcausa.com>
http://www.pcausa.com

Adapted V1.0 to be built using currently available development tools.

Application can be built using Visual Studio 2005.

Driver can be built using the Microsoft Vista Windows Driver Kit
(WDK 6000).

Removed support for Windows versions prior to Windows XP.

Extremely limited testing on Windows Vista.

No added functionality...

usbsnoop version 1.5
21/12/2001 benoit.papillault@free.fr
http://benoit.papillault.free.fr/speedtouch/sniff-src-1.5.zip

Major modifications has been made again. There was a bug were lines
printed for a URB were mixed with lines coming from another URB. This
is explained because DumpURB() is called both from lower layer (interrupt
handler for example) and from upper layer. The correction is to put lines
from DumpURB() into a buffer and then write this buffer atomically in 
the log file. So we added the following functions:

LogInit() : create the thread that create the log file
LogDone() : wait for the thread to terminate
LogBuffer() : write a buffer into the log
LogPrint()  : write a printf()-like string into the log

BufferInit()
BufferDone()
BufferPrintf() : write a printf()-like string into a buffer

Minor modifications:
The _chkesp() function has been removed. To do so, I also removed
the C++ compiler option (see Projet->Settings) /GZ.

usbsnoop version 1.4
27/11/2001 benoit.papillault@free.fr
http://benoit.papillault.free.fr/speedtouch/sniff-src-1.4.zip

Major modifications have been made. USB logs are no longer sent to the
debugger (dbgview.exe), but instead a file called "snoopy.log" is created in
your system directory (like C:\WINNT). The location of this file is currently
fixed.

A kernel thread is used to write to the log file. Each line to be printed is
sent to this thread, using a double linked list protected by a semaphore. Thus,
there are some limitations :
- memory (each line is allocated with non pageable memory). If memory
	allocation fails, a counter is incremented.
- semaphore have a limit count under Windows and an exception (ie a BSOD) is
	raised if we reach this limit. So we check the semaphore and if it fails,
	a counter is incremented

Decoding of most URB have been improved, especially ISO URB. So, after all
checks I've done, no BSOD should ever occur (tested under Win2k). So, if it
happen, drop me an email.
